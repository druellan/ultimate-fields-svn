(function($, document, window) {
	...
	*/
	...
	_.extend( UF.ContainerPostMeta, {
		listen: function() {
			// If the current screen is not a post edit screen, ignore AJAX checks to prevent
	...
	Field.extend( 'Richtext', {
		...
		initEditor: function( editor ) {
			...
			this.values[ editor.id ] = editor.$container.find( 'textarea' ).val();
		...
		regenerate: function( editor ) {
			var $parent, value, oldId = editor.id;
			...
			// Restore the value
			editor.$container.find('textarea').val(  );

			// Setup the editor
			this.initMce( editor.id, this.values[ oldId ] );
			wpActiveEditor = null
		},

		// Initialize an editor
		initMce: function( id, value ) {
			var oldId, i, that = this;

			// Get an existing ID
			// This uses the last available editor's config, but we preffer uf_dummy_editor_id
			// for( i in tinyMCEPreInit.mceInit ) { oldId = i; }

			$('#' + id).html( switchEditors._wp_Autop( value || that.values[ id ] || '' ) );
			...
			tinyMCEPreInit.mceInit[ id ].setup = function( editor ) {
				setTimeout(function() {
					that.values[ id ] = switchEditors.pre_wpautop( editor.getContent() );
				}, 300);
			...
			// Listen to changes in textareas
			$('#' + id).on( 'change keyup', function(){
				that.values[ id ] = $( this ).val();
			});
		}//,
		//
		...
		// 	// }
		//
		// 	return editor.$container.find( 'textarea' ).val();
		// }
	}, {
		...
		initSortable: function() {
			...
			this.$elements.fields.sortable({
				...
				receive: function( e, ui ) {
					...
					// Clean the row up
					$newRow.removeClass( 'uf-prototype ui-draggable ui-draggable-handle' )
						.attr( 'style', '' );
